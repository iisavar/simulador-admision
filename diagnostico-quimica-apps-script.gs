/**
 * APPS SCRIPT — DIAGNÓSTICO DE QUÍMICA
 *
 * INSTRUCCIONES:
 * 1. Crea un Google Sheet NUEVO (diferente al del simulador y al de matemáticas)
...
function inicializar() {
  ...
  sheet.setName('Diagnóstico Química');
...
function enviarCorreo_(data) {
  try {
    ...
    var pdf = HtmlService.createHtmlOutput(htmlReporte)
      ...
      .setName('Diagnostico Quimica - ' + data.nombre + '.pdf');
    ...
    var cuerpo = '<div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;">' +
      '<div style="background:#1B2A4A;color:white;padding:24px;border-radius:12px 12px 0 0;text-align:center;">' +
        ...
        '<p style="margin:6px 0 0;opacity:.7;font-size:14px;">Prueba de Química</p>' +
      '</div>' +
      '<div style="padding:24px;border:1px solid #e2e8f0;border-top:none;border-radius:0 0 12px 12px;">' +
        ...
        '<div style="text-align:center;padding:20px;background:#f8fafc;border-radius:8px;margin:16px 0;">' +
          ...
          '<p style="margin:8px 0 0;color:#64748b;">' + data.correctas + ' de ' + data.total + ' correctas</p>' +
        '</div>' +
        '<table width="100%" cellpadding="0" cellspacing="0" style="margin:0 0 20px;">' +
          '<tr>' +
            '<td width="33%" style="text-align:center;padding:12px 0;">' +
              '<div style="font-size:20px;font-weight:bold;color:#22c55e;">' + data.correctas + '</div>' +
              '<div style="font-size:12px;color:#94a3b8;margin-top:2px;">Correctas</div>' +
            '</td>' +
            '<td width="33%" style="text-align:center;padding:12px 0;border-left:1px solid #e2e8f0;border-right:1px solid #e2e8f0;">' +
              '<div style="font-size:20px;font-weight:bold;color:#ef4444;">' + (data.total - data.correctas) + '</div>' +
              '<div style="font-size:12px;color:#94a3b8;margin-top:2px;">Incorrectas</div>' +
            '</td>' +
            '<td width="33%" style="text-align:center;padding:12px 0;">' +
              '<div style="font-size:20px;font-weight:bold;color:#1a1a2e;">' + data.tiempo + '</div>' +
              '<div style="font-size:12px;color:#94a3b8;margin-top:2px;">Tiempo</div>' +
            '</td>' +
          '</tr>' +
        '</table>' +
        ...
        '<p style="color:#94a3b8;font-size:13px;">Saludos,<br><strong>Ignacio Isa</strong></p>' +
      '</div>' +
    '</div>';

    MailApp.sendEmail({
      to: data.email,
      subject: 'Diagnóstico de Química — ' + data.puntaje + ' — ' + data.nombre,
      htmlBody: cuerpo,
      attachments: [pdf],
      name: 'Ignacio Isa'
    });
...
function generarPDF_(data) {
  ...
  // Encabezado
  html += '<div class="hdr">' +
    '<h1>Prueba de Diagnóstico — Química</h1>' +
  ...
  html += '<p style="text-align:center;color:#aaa;margin-top:30px;font-size:10px;">Ignacio Isa · Diagnóstico de Química</p>';
  html += '</body></html>';
  return html;
}
