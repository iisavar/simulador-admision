...
(function () {
  ...
  function pintarRegistro(r) {
    ...
    r.innerHTML =
      ...
      '<div class="marca"><div class="marca-logo marca-q">H</div><div class="marca-txt"><b>Química desde cero</b><span>materia, átomo y tabla periódica</span></div></div>' +
      '<div style="margin:22px 0 18px">' +
      '<h1>¿De qué está hecho todo?</h1>' +
      '<p class="tinta-2" style="margin-top:8px">Empezamos desde cero: la materia, el átomo y la tabla periódica. Con simuladores para que lo veas con tus propios ojos.</p>' +
      '</div>' +
      '<ol class="estaciones" style="list-style:none;margin-bottom:18px">' +
      est('1', 'Láminas', 'Te lo explicamos todo · unos 30 min') +
      est('2', 'Juego', 'Practicas en el laboratorio · unos 30 min') +
  ...
  function pintarHub(r) {
    ...
    r.innerHTML =
      ...
      '<div class="fila"><div class="marca esp"><div class="marca-logo marca-q">H</div><div class="marca-txt"><b>Química desde cero</b><span>' + ui().esc(st.usuario.nombre) + '</span></div></div>' +
      ...
      estacion('laminas', lamHecha ? 'hecha' : 'curso', 'Láminas', lamHecha ? 'Listas ✓ · puedes repasarlas' : 'Avance ' + pctL + '% · unos 30 min', lamHecha ? ui().icon('check') : '1') +
      estacion('juego', juegoHecho ? 'hecha' : lamHecha ? 'curso' : 'bloq', 'Juego: Laboratorio', juegoHecho ? 'Camino terminado ✓' : lamHecha ? 'Practica en el laboratorio · unos 30 min' : 'Se abre al terminar las láminas', juegoHecho ? ui().icon('check') : lamHecha ? '2' : ui().icon('candado')) +
  ...
  // ---------- Glosario ----------
  const GLOSARIO = [
    ['Materia', 'Todo lo que tiene masa y ocupa un lugar en el espacio. El aire también es materia.', ''],
    ['Masa', 'La cantidad de materia que tiene un cuerpo. Se mide en gramos o kilogramos.', ''],
    ['Volumen', 'El espacio que ocupa un cuerpo. Se mide en litros o en cm³.', ''],
    ['Sustancia pura', 'Materia con una composición fija: un elemento (O₂) o un compuesto (H₂O).', ''],
    ['Mezcla', 'Dos o más sustancias juntas que no se transforman. Homogénea si se ve igual en todas partes; heterogénea si se distinguen sus partes.', ''],
    ['Elemento', 'Sustancia formada por un solo tipo de átomo. Está en la tabla periódica: Fe, O, Na…', ''],
    ['Compuesto', 'Sustancia formada por dos o más elementos unidos químicamente: H₂O, NaCl, CO₂.', ''],
    ['Cambio físico', 'La sustancia cambia de forma o de estado, pero sigue siendo la misma. Ej.: hielo que se derrite.', ''],
    ['Cambio químico', 'Se forma una sustancia nueva. Ej.: quemar papel, oxidarse un clavo.', ''],
    ['Átomo', 'La partícula más pequeña de un elemento que conserva sus propiedades.', ''],
    ['Protón (p⁺)', 'Partícula con carga positiva que está en el núcleo.', ''],
    ['Neutrón (n⁰)', 'Partícula sin carga que está en el núcleo.', ''],
    ['Electrón (e⁻)', 'Partícula con carga negativa que se mueve alrededor del núcleo. Su masa es unas 1836 veces menor que la del protón.', ''],
    ['Número atómico (Z)', 'Cantidad de protones. Dice qué elemento es.', ''],
    ['Número de masa (A)', 'Protones + neutrones. Siempre es un número entero.', ''],
    ['Isótopos', 'Átomos del mismo elemento (mismo Z) con distinta cantidad de neutrones (distinto A).', ''],
    ['Ion', 'Átomo que ganó o perdió electrones. Catión: perdió electrones (+). Anión: ganó electrones (−).', ''],
    ['Grupo', 'Columna de la tabla periódica (hay 18). Sus elementos se parecen.', ''],
    ['Periodo', 'Fila de la tabla periódica (hay 7).', '']
  ];
  function glosario() {
    ui().hoja({
      tipo: 'info', icono: 'ayuda', titulo: 'Glosario',
      html: '<dl class="glosario">' + GLOSARIO.map(g => '<dt>' + g[0] + '</dt><dd>' + g[1] + '</dd>').join('') + '</dl>',
